    ...
    def show(self, buf=sys.stdout, offset=0, attrnames=False, nodenames=False, showcoord=False, _my_node_name=None):
        ...
        if self.attr_names:
            if attrnames:
                ...
                #print('vlist type = ', type(vlist), 'value =', vlist)
                #if (['bitvector'] in vlist):
                 #   vlist = 'bitvector[' + str(vlist[1]) + ']'
                  #  attrstr = ' ' + vlist
                #else:
...
class Decl(Node):
    ...
    def children(self):
        ...
        if self.type is not None:
            ...
            #print('them vao size di nhe = ',type(self.type))
        if self.init is not None: nodelist.append(("init", self.init))
...
class IdentifierType(Node):
    __slots__ = ('names', 'coord', 'bit_size')
    ...
    def get_bit_size(self):
        if('bitvector' in str(self.names[0])):
            bitvt =  str(self.names[0])
            bitvt = bitvt[10: len(bitvt)-1]
            return bitvt
        else: return None
