...
class CParser(PLYParser):
    ...
    def p_type_specifier_no_typeid(self, p):
        ...
        coord = self._token_coord(p, 1)
        if('bitvector' in p[1]):
            #print('p2=', p[0],p[1],p[2],p[3],p[4])
            bit_size = ''
            string = str(p.slice[3])
            substring = string[string.find(',') + 2:]
            bit_size = substring[: substring.find(',') - 1]
            p[0] = c_ast.IdentifierType([str(p[1])+ '['+bit_size + ']'],  coord )
        else : p[0] = c_ast.IdentifierType([p[1]], self._token_coord(p, 1))
    ...
    def p_error(self, p):
        ...
        #
        if p:
            print('p=',p, 'column=self.clex.find_tok_column(p))= ',self.clex.find_tok_column(p));
            self._parse_error(
                'before: %s' % p.value,
